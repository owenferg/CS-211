...
# log.setLevel(logging.INFO) # For less info
...
class Board(object):
    ...
    def __init__(self):
        ...
        for col_i in range(NCOLS):
            col = [row[col_i] for row in self.tiles]
    ...
    def naked_single(self) -> bool:
        ...
        for group in self.groups:
            used_symbols = set(tile.value for tile in group if tile.value != UNKNOWN)
            ...
            for tile in group:
                if tile.value == UNKNOWN:
                    ...
                    if len(tile.candidates) == 1:
                        tile.set_value(tile.candidates.pop())
                        result = True 
        
        return result
    
    def hidden_single(self) -> bool:
        """Eliminate candidates and check for sole remaining possibilities using hidden single method.
        Return value True means we crossed off at least one candidate.
        Return value False means we made no progress.
        """
        result = False
        ...
        for group in self.groups:
            # find values not placed on any tile in group
            leftovers = set(CHOICES)
            for tile in group:
                if tile.value in CHOICES:
                    leftovers.remove(tile.value)
            
            # for each leftover value count occurrences in candidates
            for value in leftovers:
                candidates_count = sum(1 for tile in group if tile.could_be(value))
                if candidates_count == 1:
                    # if only one tile can hold value, place
                    for tile in group:
    ...
    def min_choice_tile(self) -> Tile: 
        ...
        with value UNKNOWN. 
        """
        min_candidates = float('inf') # big number to start
        min_tile = None
    ...
    def solve(self) -> bool:
        ...
        for value in tile_to_guess.candidates: # for each val the tile could hold, set it to that val
            ...
            if self.solve():
                ...
                self.set_tiles(saved_state) # restore saved state

        return False # if all possibilities dont work...
